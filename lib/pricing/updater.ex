defmodule Pricing.Updater do
  ...
  def process(%Data{} = recent) do
    ...
    # Issue only 1 db read query
    case Product.get_by(:external_product_id, recent.id) do
      nil -> # Unsuccessful db match
      ...
      # and the product is not discontinued, create a new product record for it.
        ...
        if false == recent.discontinued do
      ...
      %Product{} = p -> # Successful db match
        ...
        cond do
          ...
          p.product_name == recent.name and p.price != recent.price_cents ->
